...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Jika Mode Simulasi Aktif, gunakan logika dummy
    if (isMockMode) {
      setTimeout(() => {
    ...
    try {
        ...
        // 1. Lakukan sign-in ke Firebase Auth
        ...
        if (user) {
            ...
            if (userDoc.exists) {
                ...
                // Fallback jika dokumen user belum dibuat tapi auth berhasil
    ...
    <div className="flex h-screen w-full bg-[#f8fafc] dark:bg-[#020617] transition-colors duration-500 relative overflow-hidden">
      ...
      <div className="hidden lg:flex flex-1 bg-gradient-to-br from-indigo-700 via-blue-600 to-indigo-900 relative items-center justify-center p-12 overflow-hidden">
          ...
          <div className="relative z-10 text-center max-w-lg">
              ...
              <p className="text-indigo-100 text-lg opacity-80 font-medium">Sistem Terintegrasi untuk Efisiensi & Transparansi Akademik Madrasah.</p>
          </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 relative h-full">
        <div className="w-full max-w-sm z-10 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="text-center lg:text-left">
                <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Masuk ke Sistem</h2>
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">IMAM Management v6.2 • Private Access</p>
            </div>

            <form onSubmit={handleLogin} className="space-y-5">
                <div className="space-y-4">
                    <div className="group">
                        <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Email / Akun</label>
