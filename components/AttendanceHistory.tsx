...
const AttendanceHistory: React.FC<AttendanceHistoryProps> = ({ onBack, onNavigate, userRole }) => {
  ...
  useEffect(() => {
    ...
    if (isMockMode) {
        setTimeout(() => {
            ...
            const mockR = { id: 's1_' + selectedDate, studentId: 's1', studentName: 'ADELIA SRI SUNDARI', class: 'XII IPA 1', status: 'Terlambat', date: selectedDate, checkIn: '07:39 | +9', duha: '08:20 (Haid)', zuhur: '12:30:00', ashar: null, checkOut: null } as any;
  ...
  const parseTimeParts = (val: string | null) => {
      ...
      let timePart = String(val).split(' ')[0].substring(0, 5);
      let metaPart: string | null = null;

      if (String(val).includes('Haid')) {
          metaPart = 'haid';
      } else if (String(val).includes('|')) {
          metaPart = String(val).split('|')[1].trim();
      }
  ...
  const displayData = useMemo(() => {
    ...
    return filteredStudents.map(student => {
        ...
        // Haid dan Terlambat ditampilkan sebagai Hadir secara visual
        if (['Haid', 'Terlambat'].includes(status as any)) status = 'Hadir' as any;
  ...
  const SessionStatus = ({ label, timeRaw }: { label: string, timeRaw: string | null }) => {
      ...
      if (isFilled) {
          colorClass = 'bg-emerald-50 border-emerald-200 text-emerald-600 dark:bg-emerald-900/10 dark:border-emerald-800/50';
      }
      ...
      return (
          <div className={`flex flex-col items-center p-2 rounded-xl border flex-1 transition-all ${colorClass}`}>
              ...
              <div className="text-[9px] font-mono font-black flex flex-col items-center">
                  <span>{time}</span>
                  {meta && <span className="text-[7px] text-rose-500 font-bold tracking-tighter">| {meta}</span>}
              </div>
          </div>
      );
  };
        ...
        <div className="space-y-3">
            {loading ? (<div className="py-20 text-center flex flex-col items-center gap-2"><Loader2 className="w-8 h-8 animate-spin text-indigo-500 opacity-20" /></div>) : displayData.map((r) => (
                <div key={r.id} className="bg-white dark:bg-[#151E32] p-4 rounded-[1.8rem] border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden">
                    <div className={`absolute top-0 left-0 w-1 h-full ${r.status === 'Hadir' ? 'bg-emerald-500' : 'bg-slate-200 dark:bg-slate-700'}`}></div>
                    <div className="flex items-center justify-between mb-4 pl-1.5">
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                            <div className="w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs shrink-0 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 border border-indigo-100 dark:border-indigo-800">{(r.studentName || '?').charAt(0)}</div>
                            <div className="min-w-0"><h4 className="font-black text-[10px] uppercase truncate text-slate-800 dark:text-slate-200">{isStudent ? 'Pencatatan Hari Ini' : r.studentName}</h4><span className={`px-1.5 py-0.5 rounded text-[7px] font-black uppercase ${r.status === 'Alpha' ? 'bg-slate-100 dark:bg-slate-800 text-slate-400' : 'bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400'}`}>{r.status === 'Alpha' ? 'BELUM ADA LOG' : r.status}</span></div>
                        </div>
                    </div>
